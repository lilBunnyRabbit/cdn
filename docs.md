...
#### Types
```ts
type StorageValue = bigint | boolean | symbol | number | object | string;
type StorageValueType = "bigint" | "boolean" | "symbol" | "number" | "object" | "string";
```
#### State
```ts
interface State extends EventTarget {
  readonly storage: Map<string, StorageValue>;
  readonly elements: Map<string, StateElement>;
  readonly config: {
    useEmitOnInit: boolean;
    useChangeEvent: boolean;
    useLogs: boolean;
  };

  constructor: (config?: State['config'], initElements?: {
    [key: string]: {
      defaultValue: StorageValue;
      config: StateElement['config']
    }
  }) => State;

  set: (key: string, value: StorageValue, config: StateElement['config']) => void;
  get: (key: string) => StorageValue;
  init: (key: string, defaultValue: StorageValue, config: StateElement['config']) => StateElement;
  attach: (key: string) => StateElement;
}
```